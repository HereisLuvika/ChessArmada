...
public class ShipMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 타겟 위치 변경이 확인될 경우 이동
        if(movingShip.position != targetPosition)
        {
            Vector3 moveDir = (targetPosition - movingShip.position).normalized; // 방향 벡터
            ...
            movingShip.rotation = Quaternion.Euler(0, 0, moveAngle); //Quaternion.Euler(0, 0, moveAngle)는 각도 기반으로 회전 생성
            movingShip.position = Vector3.MoveTowards(movingShip.position, targetPosition, moveSpeed * Time.deltaTime); // MoveTowards로 선형 이동
        }
    }  
    // 함선 할당 함수, ShipBehavior에서 선택한 함선이 selectedShip이 되어 이동 함선, 현재 위치를 할당
    public void AssignShip(Transform selectedShip)
    {
        movingShip = selectedShip;
        targetPosition = movingShip.position;
    }
