...
public class ShipMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 타겟 위치 변경이 확인될 경우 이동
        if(selectedShip.position != targetPosition)
        {
            Vector3 moveDir = (targetPosition - selectedShip.position).normalized; // 방향 벡터
            ...
            selectedShip.rotation = Quaternion.Euler(0, 0, moveAngle); //Quaternion.Euler(0, 0, moveAngle)는 각도 기반으로 회전 생성
            selectedShip.position = Vector3.MoveTowards(selectedShip.position, targetPosition, moveSpeed * Time.deltaTime); // MoveTowards로 선형 이동
        }
    }  
    ...
    public void AssignShip(Transform selectedShip)
    {
        ...
        targetPosition = this.selectedShip.position;
    }
